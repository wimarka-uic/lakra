...
// Component to handle signed URLs for audio playback
const SignedAudioPlayer: React.FC<{ filePath: string; duration?: number }> = ({ filePath }) => {
  ...
  useEffect(() => {
    const getSignedUrl = async () => {
      try {
        setIsLoading(true);
        setError('');
        const url = await annotationsAPI.getSignedVoiceRecordingUrl(filePath);
        setSignedUrl(url);
      } catch (err) {
...
const MyAnnotations: React.FC = () => {
  ...
  // Check if user has completed onboarding test - moved after all hooks
  ...
  const handleEditConfirm = (annotationId: number) => {
    ...
    setEditForm({
      ...
      voice_recording_duration: annotation.voice_recording_duration,
    });
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (editForm.voice_recording_blob && updatedAnnotation) {
        try {
          logger.debug('Uploading voice recording', {
            component: 'MyAnnotations',
            action: 'upload_voice_recording',
            metadata: {
              annotationId: updatedAnnotation.id,
              blobSize: editForm.voice_recording_blob.size,
              blobType: editForm.voice_recording_blob.type,
              duration: editForm.voice_recording_duration
            }
          });
          
          await annotationsAPI.uploadVoiceRecording(
            updatedAnnotation.id,
            editForm.voice_recording_blob,
            editForm.voice_recording_duration || 0
          );
        } catch (voiceError) {
          console.error('Failed to upload voice recording:', voiceError);
          setMessage('Annotation updated, but voice recording upload failed. Please try again later.');
  ...
  const handleVoiceRecordingComplete = (audioBlob: Blob, duration: number) => {
    logger.debug('handleVoiceRecordingComplete', {
      component: 'MyAnnotations',
                      ...
                      {annotation.voice_recording_url && (
                        <div className="border-t pt-4">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <h5 className="text-xs font-medium text-gray-700">Voice Recording</h5>
                            {annotation.voice_recording_duration && (
                              <span className="text-xs text-gray-500">
                                {Math.floor(annotation.voice_recording_duration / 60)}:{(annotation.voice_recording_duration % 60).toString().padStart(2, '0')}
                              </span>
                            )}
                          </div>
                          <SignedAudioPlayer 
                            filePath={annotation.voice_recording_url}
                            duration={annotation.voice_recording_duration}
                          />
                        </div>
                      )}
