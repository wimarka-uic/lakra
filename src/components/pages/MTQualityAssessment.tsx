...
interface SentenceAnnotation {
  sentence_id: number;
  fluency_score?: number; // Stored as 1-5 scale in database, displayed as percentage (20%-100%)
  adequacy_score?: number; // Stored as 1-5 scale in database, displayed as percentage (20%-100%)
  overall_quality?: number; // Stored as 1-5 scale in database, displayed as percentage (20%-100%)
  comments: string;
  final_form: string;
...
const MTQualityAssessment: React.FC = () => {
        ...
        if (sentenceId) {
          ...
          try {
            ...
            if (existingAnnotation) {
              ...
              // Load existing annotation data (keep 1-5 scale for database compatibility)
              ...
              initialAnnotations.set(specificSentence.id, {
                ...
                fluency_score: existingAnnotation.fluency_score, // Keep as 1-5 scale
                adequacy_score: existingAnnotation.adequacy_score, // Keep as 1-5 scale
                overall_quality: existingAnnotation.overall_quality, // Keep as 1-5 scale
  ...
  // Convert percentage to 1-5 scale for database storage
  const percentageToScale = (percentage: number): number => {
    return Math.round(percentage / 20); // 20% = 1, 40% = 2, 60% = 3, 80% = 4, 100% = 5
  };

  // Convert 1-5 scale to percentage for display
  const scaleToPercentage = (scale: number): number => {
    return scale * 20; // 1 = 20%, 2 = 40%, 3 = 60%, 4 = 80%, 5 = 100%
  };
  ...
  const handleRatingChange = (sentenceId: number, field: 'fluency_score' | 'adequacy_score' | 'overall_quality', value: number) => {
    setAnnotations(prev => {
      ...
      const annotation = updated.get(sentenceId);
      if (annotation) {
        // Store as 1-5 scale in the annotation object (for database compatibility)
        annotation[field] = percentageToScale(value);
        updated.set(sentenceId, annotation);
      }
      return updated;
    });
  ...
  const validateAnnotation = (annotation: SentenceAnnotation) => {
    ...
    // Check if any scores are provided (1-5 scale)
    if (!annotation.fluency_score && !annotation.adequacy_score && !annotation.overall_quality) {
      errors.push('Please rate the translation quality by clicking on the percentage buttons (20%-100%) in at least one category below.');
    }
  ...
  const RatingButtons: React.FC<{ 
    ...
    // Convert 1-5 scale to percentage for display
    const displayValue = value ? scaleToPercentage(value) : undefined;
    
    return (
      <div className={compact ? "space-y-1" : "space-y-2"}>
        <label className={`block text-xs font-medium ${
          displayValue ? 'text-gray-700' : showRequired ? 'text-red-600' : 'text-amber-600'
        } ${compact ? 'text-center' : ''}`}>
          {label} {!displayValue && showRequired && <span className="text-red-500">*</span>}
          {!displayValue && !showRequired && <span className="text-amber-500">*</span>}
        </label>
        ...
        {!displayValue && !showRequired && (
          <p className="text-xs text-amber-600 mt-1">Click a number to rate (20%=poor, 100%=excellent)</p>
        )}
      </div>
    );
  };
    ...
    <div className="max-w-full mx-auto space-y-4 sm:space-y-6">
      <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-3 sm:space-y-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Model Prediction Review</h1>
        ...
        <div className="mb-4 sm:mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-3 sm:p-4">
          <h3 className="text-sm font-semibold text-blue-900 mb-2">📋 How to review model predictions:</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 text-xs text-blue-800">
            ...
            <div className="flex items-start space-x-2">
              ...
              <div>
                ...
                <p className="text-blue-700">Tap a percentage from 20% (poor) to 100% (excellent) for at least one category</p>
              </div>
            </div>
            <div className="flex items-start space-x-2">
              ...
              return (
                <>
                  <div className="flex items-center mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">
                        {hasWarnings ? 'Ready to submit?' : 'Submit your assessment'}
                      </h3>
                      <p className="text-sm text-gray-600">
                        {hasWarnings ? 'Please confirm your submission' : 'Your assessment is ready to be submitted'}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div>
                        ...
                        <ul className="mt-1 space-y-1 text-gray-800">
                          {annotation.fluency_score && <li>• Fluency: {scaleToPercentage(annotation.fluency_score)}%</li>}
                          {annotation.adequacy_score && <li>• Adequacy: {scaleToPercentage(annotation.adequacy_score)}%</li>}
                          {annotation.overall_quality && <li>• Overall: {scaleToPercentage(annotation.overall_quality)}%</li>}
                        </ul>
                      </div>
                      <div>
