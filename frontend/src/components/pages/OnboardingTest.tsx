...
const OnboardingTest: React.FC = () => {
  ...
  const handleSubmitTest = useCallback(async () => {
    ...
    try {
      ...
      logger.debug('Test submission result', {
        component: 'OnboardingTest',
        action: 'test_submission_result',
        metadata: { result }
      });
            ...
            {user?.languages && user.languages.length > 0 ? (
              <div className="text-center">
                ...
                <p className="mt-3 text-sm text-gray-500">
                  By starting the quiz, the timer will begin (90 seconds per question)
                </p>
              </div>
            ) : (
              <div className="text-center">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className={`flex items-center px-3 py-1 rounded-full text-sm font-medium ${
              ...
              {formatTime(timeRemaining)} ({timeRemaining}s)
            </div>
